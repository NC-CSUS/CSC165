...
public class GameServerTCP extends GameConnectionServer<UUID> 
{
	...
	public void processPacket(Object o, InetAddress senderIP, int senderPort)
	{
		...
		{	// Case where client just joined the server
			...
			*/
			...
			// Case where server receives a MORE-DETAILS-FOR message
			// Received Message Format: (dsfr,remoteId,localId,x,y,z)
			if(messageTokens[0].compareTo("dsfr") == 0)
			{	UUID clientID = UUID.fromString(messageTokens[1]);
				UUID remoteID = UUID.fromString(messageTokens[2]);
	 ...
	 * remoteId is used to send this message to the proper client. 
	 * <p>
	 * Message Format: (dsfr,remoteId,x,y,z) where x, y, and z represent the position. po[3] is the rotation around Y
	 */
	public void sendMoreDetailsForMessage(UUID clientID, UUID remoteId, String[] position)
	{	try 
		{	String message = new String("dsfr," + remoteId.toString());
			message += "," + position[0];
			message += "," + position[1];
